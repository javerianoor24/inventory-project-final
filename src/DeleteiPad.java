...
class DeleteiPad extends JFrame {
    ...
    public DeleteiPad() {
        ...
        JLabel phoneNameLabel = new JLabel("iPad Name:");
    ...
    private void processDelete() {
        ...
        if (phoneName.isEmpty()) {
            JOptionPane.showMessageDialog(this, "Please fill in the phone name.", "Input Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try (Connection connection = DriverManager.getConnection(url);
            ...
            String deleteQuery = "DELETE FROM tablets WHERE name = '" + phoneName + "'";
            int rowsAffected = statement.executeUpdate(deleteQuery);

            if (rowsAffected > 0) {
                JOptionPane.showMessageDialog(this, "iPad deleted successfully!", "Success", JOptionPane.INFORMATION_MESSAGE);
                phoneNameField.setText("");  // Clear the input field after deletion
            } else {
                JOptionPane.showMessageDialog(this, "iPad not found or could not be deleted.", "Error", JOptionPane.ERROR_MESSAGE);
            }
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            DeleteiPad frame = new DeleteiPad();
